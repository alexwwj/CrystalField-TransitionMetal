function Chi = SingleIonMagSusceptibility(AtomCEF,T,Chi_dia,lambda)
...
mask_VanVleck   = double(abs(deltaE)<1e-10);
...
ChiIon  =  permute(reshape(factor * real(Chi_Curie + Chi_VanVleck),3,3,nT),[2,1,3]);
% Chi_ion(Chi_ion<1e-12) = 0;

ChiPow = reshape((ChiIon(1,1,:) + ChiIon(2,2,:) + ChiIon(3,3,:))/3,1,nT);
% lambda = 23.99;
ChiPowMF = (Chi_dia + ChiPow ./ (1+lambda*ChiPow));

Chi.ChiIon = ChiIon;
Chi.ChiPowMF = ChiPowMF;
