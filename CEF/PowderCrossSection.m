function SpecPowd = PowderCrossSection(AtomCEF,T,Ei,Q,Omega,p)
...
BolzmanFac  = repmat(Bolzman(DiffE,DiffE,T),1,N,3,nQ); % dim [N N 3 nQ]
mfac        = repmat(reshape(fac,1,1,1,nQ),N,N,3,1);     % dim [N N 3 nQ]
EigVec1     = repmat(CEFvec',1,1,3,nQ);        % dim [N N 3 nQ]
EigVec2     = repmat(CEFvec,1,1,3,nQ);         % dim [N N 3 nQ]
A           = gpuArray(Mperp);
...
FWHM  =  p(1) * omega.^3 + p(2) * omega.^2 + p(3) * omega.^1 + p(4); 
Gamma =  1/2 * FWHM;
