function EulerAngle = EulerAngleCalc(Z,X,p,n,pm,tol)

if pm ~= 0 && pm ~= 1
    disp('pm has to be either 0 (+) or 1 (-)')
    return
end
...
EulerAngle(1) = thetaZ1 * (thetaZ1>=0 && thetaZ1<2*pi) + (2 * pi + thetaZ1) * (thetaZ1<0);
EulerAngle(2) = thetaY;
EulerAngle(3) = thetaZ0 * (thetaZ0>=0 && thetaZ0<2*pi) + (2 * pi + thetaZ0) * (thetaZ0<0);

indx = abs(EulerAngle-2*pi)<tol;
EulerAngle(indx) = 0;
